...
describe(`${chalk.yellow.bold.underline(
  'Inheritance'
)}: applying "${chalk.blue.bold('keys')}" and "${chalk.blue.bold(
  'types'
)}" restrictions`, () => {
  let Groups;
  let Roles;
    ...
    describe(`Group: "${chalk.blue.bold(demoGroup)}"`, () => {
      ...
      allRoleKeys.forEach(key => {
        ...
        if (demoGroupRestrictedKeys.length) {
          describe(`inheriting "${chalk.yellow.bold(
            key
          )}" with restrictedKeys [${chalk.blue.italic(
            demoGroupRestrictedKeys
          )}] `, () => {
            const modifiedBaseRoleKeys = Object.keys(
            ...
            it(`expects definitions to ${chalk.red.bold.underline(
              'not'
              ...
              const demoGroupKeys = Groups[symbolize(demoGroup)][
                ...
                [_permittedKeys]()
                .sort();
              expect(demoGroupKeys).toEqual(
                expect.not.arrayContaining(demoGroupRestrictedKeys)
              );
            });
          });
        }
