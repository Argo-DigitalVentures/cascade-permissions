...
describe(`${chalk.yellow.bold.underline(
  'Methods'
)}: removing applied "${chalk.blue.bold('keys')}" and "${chalk.blue.bold(
  'types'
)}" restrictions`, () => {
  let Groups;
  let Roles;
    ...
    describe(`Group: "${chalk.blue.bold(demoGroup)}"`, () => {
      ...
      allRoleKeys.forEach(key => {
        ...
        if (demoGroupRoleRestrictedKeys.length) {
          describe(`removing "${chalk.yellow.bold(
            key
          )}" restrictedKeys [${chalk.blue.italic(
            demoGroupRoleRestrictedKeys
          )}] `, () => {
            const baseRoleKeys = Object.keys(BaseRoles()[key]()).sort();
            it(`expects initial definitions to ${chalk.red.bold.underline(
              'not'
            )} contain [${chalk.blue.bold.italic(
              demoGroupRoleRestrictedKeys
            )}]`, () => {
              const demoGroupRoleKeys = Groups[symbolize(demoGroup)][
                symbolize('roles')
              ][symbolize(key)]
                [_permittedKeys]()
                .sort();
            ...
            it(`expects post-removal definitions to match [${chalk.blue.bold.italic(
              baseRoleKeys
            )}]`, () => {
              const demoGroupRole =
              ...
              const demoGroupPreRoleKeys = demoGroupRole[
                _permittedKeys
              ]().sort();
              demoGroupRole[_restrictedOwnKeysRemove](
                demoGroupRoleRestrictedKeys
              );
              const demoGroupPostRoleKeys = demoGroupRole[
                _permittedKeys
              ]().sort();

              expect(demoGroupPreRoleKeys).toEqual(
                expect.not.arrayContaining(demoGroupRoleRestrictedKeys)
              );
        ...
        if (
          ...
          describe(`restricted "${chalk.green.bold(key)}"`, () => {
            it(`expects initial "${chalk.yellow.bold(
              key
            )}" to be ${chalk.red.bold('undefined')}`, () => {
              const group = Groups[symbolize(demoGroup)][symbolize('roles')];
              const groupRole = group[symbolize(key)];
              expect(groupRole).toBeUndefined();
            });
            it(`expects post-removal type restriction "${chalk.yellow.bold(
              key
            )}" to be ${chalk.green.bold('defined')}`, () => {
              const group = Groups[symbolize(demoGroup)][symbolize('roles')];
        ...
        if (demoGroupTypeRestrictedKeys.length) {
          describe(`removing "${chalk.yellow.bold(
            ...
            it(`expects initial definitions to ${chalk.red.bold.underline(
              ...
              const demoGroupTypeKeys = Groups[symbolize(demoGroup)][
                symbolize('types')
              ][symbolize(key)]
                [_permittedKeys]()
                .sort();
            ...
            it(`expects post-removal definitions to match [${chalk.blue.bold.italic(
              ...
              const demoGroupPreTypeKeys = demoGroupType[
                _permittedKeys
              ]().sort();
              demoGroupType[_restrictedOwnKeysRemove](
                demoGroupTypeRestrictedKeys
              );
              const demoGroupPostTypeKeys = demoGroupType[
                _permittedKeys
              ]().sort();

              expect(demoGroupPreTypeKeys).toEqual(
                expect.not.arrayContaining(demoGroupTypeRestrictedKeys)
              );
