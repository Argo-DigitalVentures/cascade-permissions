...
// begin setup ///////////////
//////////////////////////////
const { _defineType, _inherit, _permittedKeys, _permittedTypes } = appSymbols;
...
export default function getPermissions(role, businessUnit = 'admin') {
  try {
    const buRoleTypes = Groups[symbolize(businessUnit)][symbolize('roles')][symbolize(role)][symbolize('types')];
    return buRoleTypes[_permittedTypes]().reduce(
      ({ fields, types }, permittedType) => ({
        types: [...types, pluralize(desymbolize(permittedType))],
        fields: {
          ...fields,
          [pluralize(desymbolize(permittedType))]: buRoleTypes[permittedType][_permittedKeys]().sort(),
        },
      }),
      {
        fields: {},
    ...
    throw new Error(`getPermissions: Error - businessUnit[${businessUnit}] role[${role}]`);
  }
}
console.log('what is data', getPermissions('basic'));
//   demoGroups.forEach(demoGroup => {
...
//             const modifiedBaseKeys = Object.keys(baseObjects[testType]()[key]()).sort();
//             it(`expects definitions to match [${chalk.blue.bold.italic(modifiedBaseKeys)}]`, () => {
