...
export default function ChildDescriptors(
  ...
  function createChildDescriptors(
    name,
    { restrictedKeys = [], restrictedTypes = [] } = {},
    prototype = this
  ) {
    const args = [
      `created "childDescriptors" for "${desymbolize(
        name
      )}" with a previous propId ${desymbolize(prototype[_propId])}`,
      caller,
      subject,
    ];
    return {
