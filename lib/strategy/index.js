...
export default function SelectStrategy(message, caller = defaultCaller, subject = defaultSubject, { color = 'blue', bold = true } = {}) {
  function _selectStrategy(prototype) {
    if (!prototype) {
      return Create().bind(null, {});
    } else {
      const inhertableTypes = Object.getOwnPropertySymbols(prototype).filter(symbol => !Object.values(appSymbols).includes(symbol));
  ...
  return new Proxy(_selectStrategy, LogHandler(message, caller, subject, { color, bold }));
}
