...
export default function TypeHandler(prototype = {}, source) {
  const restrictedKeys = prototype[_restrictedKeys] ? prototype[_restrictedKeys]() : Object.keys(source);
  return {
    get(target, key, context) {
      return restrictedKeys.includes(key) ? undefined : Reflect.get(target, key, context);
    },
    ...
    ownKeys(target) {
      return Reflect.ownKeys(target).filter(key => !restrictedKeys.includes(key));
    },
  };
}
