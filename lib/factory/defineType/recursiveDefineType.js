...
export default function recursiveDefinedType(
  name,
  restricted = {},
  prototype = this
) {
  ...
  const defaultCloneMessage = `"${desymbolize(
    this[_propId]
  )}" is cloning ${desymbolize(symbolizeName)}`;

  return prototype[_permittedTypes]()
    .filter(type => !restrictedTypesList.includes(type))
    .reduce((col, permittedType) => {
      const defaultInheritMessage = `"${desymbolize(
        symbolizeName
      )}" is inheriting definedType "${desymbolize(permittedType)}"`;
