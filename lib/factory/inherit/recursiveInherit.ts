...
import { desymbolize, getUniqueArrayItems, isObject, symbolize, validateObject } from '../../util';
  ...
  return prototype[_permittedTypes]().reduce((col, permittedType) => {
    if (!restrictedTypesList.includes(permittedType)) {
      ...
      if (shouldInheritDefinedType) {
        if (isObject(restrictedKeys)) {
          // console.log(`restrictedKeys`, restrictedKeys);
        }
        if (isObject(restrictedKeys) && restrictedKeys[desymbolize(permittedType)]) {
          ...
          // console.log(`${desymbolize(name)}: restrictedKeys`, restrictedKeys[desymbolize(permittedType)]);
        }
