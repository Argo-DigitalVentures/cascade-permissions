...
export default function BaseFactory(
  ...
  validateArray(
    filename,
    'restricted.restrictedKeys',
    restricted.restrictedKeys || []
  );

  const defaultSubject = `BaseFactory is selecting strategy for "${desymbolize(
    name
  )}" object`;
  return Object.defineProperties(
    selectStrategy(message, caller, subject || defaultSubject, { color, bold })(
      prototype
    )(restricted, name),
    {
      ...DefineType(message),
